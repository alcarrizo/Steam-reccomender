...
gameData = []
games = ["American Truck Simulator", "Portal", "Portal 2", "Deponia"]


for game in games:
    gameData.append(getGameInfo(game, df))
...
#targetGame = gameData[2]
targetGame = getGameInfo("Deponia", df)
game = ", ".join(targetGame["tags"])
# targetGame2 = getGameInfo("Garry's Mod", df)
# game2 = ", ".join(targetGame2["tags"])
...
# print(cosine_similarity(TfidfVec.fit_transform(
#    game), TfidfVec.fit_transform(game2)))


vals = cosine_similarity(tfidf[-1], tfidf)

idx = []
# getting the games that meet a minimum similarity score.
# i = 0
# while vals[0][vals.argsort()[0][-3 - i]] >= .53:
...
# print(response(user_response, df, game))
#print(gameData[2]["title"] + "\n" + ",".join(gameData[2]["tags"]) + "\n")
similarGames = []
for i in idx:
...
# print(userDf.loc[0]["items"][0]["item_name"])
